...
const char* plugin_name = "ResurrectionScroll.esp";
const int f314RM_Mgef = 0x807;
const int f314RM_Spel = 0x809;
const int f314RM_scrol = 0x805;

class ScrollResurrection : public ResurrectionAPI
{
	bool should_resurrect(RE::Actor* a) const override
	{
		static auto item = RE::TESDataHandler::GetSingleton()->LookupForm<RE::TESObjectMISC>(f314RM_scrol, plugin_name);
		static auto mgef = RE::TESDataHandler::GetSingleton()->LookupForm<RE::EffectSetting>(f314RM_Mgef, plugin_name);
		return FenixUtils::get_item_count(a, item) && !a->HasMagicEffect(mgef);
	}
	...
	void resurrect(RE::Actor* a) override
	{
		...
		a->RemoveItem(item, 1, RE::ITEM_REMOVE_REASON::kRemove, nullptr, nullptr);
	}
};
...
void addSubscriber()
{
	if (auto pluginHandle = GetModuleHandleA("ResurrectionAPI.dll")) {
		if (auto AddSubscriber = (AddSubscriber_t)GetProcAddress(pluginHandle, "ResurrectionAPI_AddSubscriber")) {
			AddSubscriber(std::make_unique<ScrollResurrection>());
		}
	}
}
